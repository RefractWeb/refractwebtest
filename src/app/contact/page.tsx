...
const ContactPage = () => {
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-background">
      ...
      {/* Logo Strip */}
      <div className="relative z-10 w-full mt-auto py-12 border-t border-border bg-black/20 backdrop-blur-sm">
        <div className="container mx-auto px-6 overflow-hidden">
          <div className="flex flex-wrap justify-center lg:justify-between items-center gap-8 lg:gap-12 opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
            {TECH_STACK.map(({ icon: Icon, label, showIcon }) => (
              <div
                key={label}
                className="flex items-center gap-2 group cursor-default"
              >
                {Icon && showIcon && (
                  <Icon className="fill-foreground text-foreground w-5 h-5" />
                )}
